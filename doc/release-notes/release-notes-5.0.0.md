Passion Core version *v5.0.0* is now available from:  <https://github.com/passion-project/passion/releases>
...
Passion Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, Passion Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run Passion v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/passion` folder to the `%APPDATA%\PassionParams` directory.
...
#### Overview
Users will be able to protect their financial information by sending and receiving PASSIONs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://passion.org .
...
* Top bar showing shield PASSION balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield PASSION balance.
...
See the release notes from [v4.2.0](https://github.com/Passion-Project/Passion/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed PASSION supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in PASSION
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in PASSION
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zPASSIONsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield PASSION)
  The `moneysupply` JSON return entry is the total PASSION supply (the sum of all transparent and shield PASSION)
...
| `getaccountaddress` | Returned the current Passion address associated with an internal account | Account system removal |
| `setaccount` | Associate a Passion address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given Passion address | Account system removal |
| `getaddressesbyaccount` | Returned a list of Passion addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a Passion address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a Passion address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zPASSION balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zPASSION mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zPASSION mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zPASSION amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zPASSION to a PASSION address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zPASSION mints to a PASSION address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zPASSIONs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzpivseed` | Set the wallet's deterministic zPASSION seed to a specific value | Zerocoin deprecation |
| `getzpivseed` | Return the current deterministic zPASSION seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zPASSION seed | Zerocoin deprecation |
| `dzpivstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzpiv` | Do a search for deterministically generated zPASSION that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_passion unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake passiond and unit tests: fixing missing MacOS framework. (furszy)

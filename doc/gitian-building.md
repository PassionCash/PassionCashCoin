...
*Setup instructions for a gitian build of Passion Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Passion
...
to the Passion GitHub release page.
...
- [Building Passion Core](#building-passion-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/passion-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/passion-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/passion-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/passion-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/passion-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/passion-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/passion-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@passion.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/passion-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/passion-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/passion-osx-*-build.assert
```

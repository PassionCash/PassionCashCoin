...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-PASSION in, 0.4 shielded-PASSION out, 0.1 t-PASSION fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling) {
    ...
    // --- 0.4 shielded-PASSION in, 0.299 shielded-PASSION out, 0.1 shielded-PASSION fee, 0.001 shielded-PASSION change (added to fee)
    ...
    // --- Now try with 1 shielded-PASSION in, 0.5 shielded-PASSION out, 0.1 shielded-PASSION fee, 0.4 shielded-PASSION change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 PASSION note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-PASSION out, 0.1 t-PASSION fee
    auto builder = TransactionBuilder(consensusParams, 1);
    ...
    // 0.5 t-PASSION out, 0.1 t-PASSION fee
    ...
    // 0.5 t-PASSION out, 0.1 t-PASSION fee, 0.59999 shielded-PASSION in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());

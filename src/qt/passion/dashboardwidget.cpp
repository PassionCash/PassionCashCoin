...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PassionGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair PASSION, zPASSION
...
void DashboardWidget::combineUTXOs() {
    ...
    if(sendValue > nSplit || coinControl->QuantitySelected() > 500) {
        ...
        walletModel->getNewAddress(dest,"Merge UTXO Transaction");
        SendCoinsRecipient sendCoinsRecipient(
                    QString::fromStdString(dest.ToString()),
                    QString::fromStdString("Merge UTXO Transaction"),
                    sendValue - 1500000,
                    "Merged UTXOs");
